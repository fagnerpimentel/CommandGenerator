...
class CommandGenerator:
        ...
        if "loc2" in command_string:
            ...
            command_string = command_string.replace(
                ...
                random.choice(
                    [
                        x
                        for x in self.placement_location_names
                        if x not in command_string
                    ]
                ),
            )
    ...
    def generate_command_followup(self, type, cmd_category="", difficulty=0):
        ...
        if command == "findObj":
            ...
            command_string = (
                "{deliverVerb} it {deliverPrep} {name} {inLocPrep} the {room}"
            )
            ...
            command_string = "{takeVerb} it and " + self.generate_command_followup(
                "hasObj"
            )
        else:
            warnings.warn("Command type not covered: " + command)
    ...
    def insert_placeholders(self, ph):
        ...
        if ph == "goVerb":
            ...
            return (
                ...
                + " the "
                + random.choice(self.location_names)
            )
